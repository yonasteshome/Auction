...
function normalizeProduct(item: Record<string, unknown>): VendorProduct {
  return {
    ...
    stock_count: item.stock_count !== undefined ? Number(item.stock_count) : undefined,
    availability:
...
function normalizeOrder(item: Record<string, unknown>): VendorOrder {
  return {
    ...
    listing_id: item.listing_id ? String(item.listing_id) : item.vendor_price ? String(item.vendor_price) : undefined,
    vendor_id: item.vendor_id ? String(item.vendor_id) : undefined,
    customer_name: item.customer_name ? String(item.customer_name) : undefined,
    customer_email: item.customer_email ? String(item.customer_email) : undefined,
...
export async function getCurrentUserProfile(): Promise<VendorProfile> {
  ...
  return {
    ...
    wallet_balance: vendorInfo?.wallet_balance != null ? Number(vendorInfo.wallet_balance) : undefined,
    wallet_currency: vendorInfo?.wallet_currency ? String(vendorInfo.wallet_currency) : undefined,
    wallet_updated_at: vendorInfo?.wallet_updated_at ? String(vendorInfo.wallet_updated_at) : undefined,
...
export async function getVendorOrders(): Promise<VendorOrder[]> {
  const data = await requestJson<unknown>("/api/ecommerce/vendor/sales/", {
    method: "GET",
  });
  ...
  if (data && typeof data === "object" && "results" in data && Array.isArray((data as any).results)) {
    return (data as any).results.map((item: any) => normalizeOrder(item));
  }
...
export async function getVendorOrderDetail(orderId: string): Promise<VendorOrder> {
  const data = await requestJson<Record<string, unknown>>(`/api/ecommerce/vendor/sales/${orderId}/`, {
    method: "GET",
  });
